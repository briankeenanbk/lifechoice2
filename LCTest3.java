...
public class LCTest3 {
    ...
    public void extraMortality() {
        assertEquals("Extra mortality", 7.73, policy.getResult().getExtraMortalityCost(), 0.01);
    }
    ...
    public void totalRatings() {
        assertEquals("Total ratings", 7.73, policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }

    @Test
    public void totalPremium() {
        assertEquals("Total premium", 91.20, policy.getResult().getUnratedPremium() + policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }

    @Test
    public void totalPremiumIncludingLevy() {
        assertEquals("Total premium", 92.11, policy.getResult().getUnratedPremium() + policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost() + ((policy.getResult().getUnratedPremium() + policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost()) * .01), 0.01);
    }

    @Test
    public void priceMatch(){
        PriceMatch pm = new PriceMatch(policy, 50);
        Policy policy = pm.priceMatch();
        assertEquals("Price Match Total premium", 57.72, policy.getResult().getUnratedPremium() + policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }
