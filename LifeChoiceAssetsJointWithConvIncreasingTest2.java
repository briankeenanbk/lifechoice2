...
public class LifeChoiceAssetsJointWithConvIncreasingTest2 {
    ...
    public static void setup() {
        DateTime policyStartDate = stringToDate("21/03/2018");
        DateTime dob1 = stringToDate("05/02/1971");
        DateTime dob2 = stringToDate("03/01/1972");
    ...
    private static BenefitInputs setupBenefitLSODInputsLife2(Policy policy) {
        ...
        benefitInputs.setRates(lsodRates);
        benefitInputs.setExtraMortalityRate(25);
        benefitInputs.setPerMilleRate(10);
        benefitInputs.setRatingTerm(10);
        benefitInputs.setCommissionFactors(commissionFactors);
        benefitInputs.setPolicyFee(5.90);
        benefitInputs.setConversionFactor(1.06);
    ...
    public void unratedPremium() {
        assertEquals("Unrated premium", 485.15, policy.getResult().getUnratedPremium(), 0.01);
    }

    @Test
    public void extraMortality() {
        assertEquals("Extra mortality", 61.12, policy.getResult().getExtraMortalityCost(), 0.01);
    }

    @Test
    public void perMille() {
        assertEquals("Per mille", 50.65, policy.getResult().getPerMilleCost(), 0.01);
    }

    @Test
    public void totalRatings() {
        assertEquals("Total ratings", 111.77, policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }

    @Test
    public void totalPremium() {
        assertEquals("Total premium", 596.92, policy.getResult().getUnratedPremium() + policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }
}
