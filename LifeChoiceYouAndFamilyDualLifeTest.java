...
public class LifeChoiceYouAndFamilyDualLifeTest {
   ...
   private static PolicyInputs setupPolicyInputs(DateTime policyStartDate) {
    ...
    public void unratedPremium() {
        assertEquals("Unrated premium", 327.82, policy.getResult().getUnratedPremium(), 0.01);
    }
    ...
    public void totalRatings() {
        assertEquals("Total ratings", 33.71, policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }

    @Test
    public void totalPremium() {
        assertEquals("Total premium", 361.53, policy.getResult().getUnratedPremium() + policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }
}
