...
public class LifeChoiceYouAndFamilySingleWithBrokenBones {
    private static final RatesMap lsodRates = new RatesMapFromFileSystem("TestData/LumpSumOnDeathRates.csv");
    private static final RatesMap brokenBonesRates = new RatesMapFromFileSystem("TestData/BrokenBonesRates.csv");
    ...
    public static void setup() {
        ...
        benefitInputs = setupBenefitBrokenBonesInputs(policy);
    ...
    private static BenefitInputs setupBenefitBrokenBonesInputs(Policy policy) {
        BenefitInputs benefitInputs = new BenefitInputs();
        benefitInputs.setPolicy(policy);
        benefitInputs.setId(BenefitType.BROKEN_BONES);
        benefitInputs.setSumAssured(0.00);
        benefitInputs.setMaxAge(65);
        benefitInputs.setRates(brokenBonesRates);
    ...
    public void unratedPremium() {
        assertEquals("Unrated premium", 61.32, policy.getResult().getUnratedPremium(), 0.01);
    }

    @Test
    public void extraMortality() {
        assertEquals("Extra mortality", 1.08, policy.getResult().getExtraMortalityCost(), 0.01);
    }
    ...
    public void totalRatings() {
        assertEquals("Total ratings", 1.08, policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }

    @Test
    public void totalPremium() {
        assertEquals("Total premium", 62.41, policy.getResult().getUnratedPremium() + policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }
