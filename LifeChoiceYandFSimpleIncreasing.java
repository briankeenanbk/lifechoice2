...
public class LifeChoiceYandFSimpleIncreasing {
    ...
    private static BenefitInputs setupBenefitLSODInputs(Policy policy) {
        ...
        benefitInputs.setSumAssured(250000);
    ...
    public void unratedPremium() {
        assertEquals("Unrated premium", 140.55, policy.getResult().getUnratedPremium(), 0.01);
    }
    ...
    public void totalPremium() {
        assertEquals("Total premium", 140.55, policy.getResult().getUnratedPremium() + policy.getResult().getExtraMortalityCost() + policy.getResult().getPerMilleCost(), 0.01);
    }
